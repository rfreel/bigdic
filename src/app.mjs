...
let pendingSave=Promise.resolve(),backupStarted=false;
...
async function handleClick(event){const b=event.target.closest('button');if(!b)return;
 ...
 case 'backup-button':download('bigdic-history.json',blocked?(storageRaw||'null'):JSON.stringify(pack(),null,2));backupStarted=true;notify('Backup download started. Check that the file is saved before starting a new workspace.');break;
 case 'new-workspace':if(!backupStarted)throw new Error('Start a backup download first.');if(!window.confirm('Confirm that the history backup is saved. Start an empty workspace?'))break;localStorage.removeItem(key);events=[];storageRaw=null;project=emptyProject();backupStarted=false;blocked=false;await save(project,'Created new workspace');renderBuild();notify('New workspace started. Keep your downloaded history backup.');break;
